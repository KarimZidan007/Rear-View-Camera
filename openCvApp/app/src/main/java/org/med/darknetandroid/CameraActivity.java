...
public class CameraActivity extends AppCompatActivity implements CvCameraViewListener2 {
    ...
    protected void onCreate(Bundle savedInstanceState) {
     ...
     //   mOpenCvCameraView.setCameraIndex(CameraBridgeViewBase.CAMERA_ID_BACK); // Back or front camera
        mOpenCvCameraView.setVisibility(CameraBridgeViewBase.VISIBLE);
        mOpenCvCameraView.setCvCameraViewListener(this);
        classNames = readLabels("labels.txt", this);
        Log.i("loading", "Onceate success ");
    ...
    public void onResume() {
        ...
        if (!OpenCVLoader.initDebug()) {
            Log.i("loading", "OpenCV on resuem error ");
            Log.d(TAG, "Internal OpenCV library not found. Using OpenCV Manager for initialization");
            OpenCVLoader.initAsync(OpenCVLoader.OPENCV_VERSION_3_4_0, this, mLoaderCallback);
        } else {
            Log.d(TAG, "OpenCV library found inside package. Using it!");
            Log.i("loading", "OpenCV on resuem success ");
    ...
    public Mat onCameraFrame(CvCameraViewFrame inputFrame) {
        Log.i("loading", "on camera frame success ");

        Mat frame = inputFrame.rgba();
        Log.i("loading", "frame allocated success");

        Imgproc.cvtColor(frame, frame, Imgproc.COLOR_RGBA2RGB);
        Size frame_size = new Size(416, 416);
        Scalar mean = new Scalar(127.5);
        if (frame.empty()) {
            Log.i("loading", "Input frame is empty.");
            return frame;
        }
        Mat blob = Dnn.blobFromImage(frame, 1.0 / 255.0, frame_size, mean, false, false);
        Log.i("loading", "Blob allocated success");

        if (blob.empty()) {
            Log.i("loading", "Failed to create blob from image.");
            return frame;
        }
        //save_mat(blob);
        net.setInput(blob);
        ...
        try {
            ...
            Log.i("loading", "Forward pass success ");

        } catch (Exception e) {
            Log.i("loading", "Error during forward pass: " + e.getMessage());
